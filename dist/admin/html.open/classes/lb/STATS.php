...
class STATS {
	...
	function parse_litespeed() {
		...
		if (strpos($_SERVER['LS_PLATFORM'], 'freebsd') !== FALSE) {
			...
			$cmd = 'status:rpt=summary';
			$content = Service::retrieveCommandData($cmd);
			if ($content == FALSE) {
				return;
			}
		}
		...
		$found = preg_match_all("/REQ_RATE \[([^\]]*)\]: REQ_PROCESSING: ([0-9]+), REQ_PER_SEC: ([0-9]+\.[0-9]), TOT_REQS: ([0-9]+), CACHE_HITS_PER_SEC: ([0-9]+\.[0-9]), TOTAL_CACHE_HITS: ([0-9]+)/i",$content,$result);
		for($f = 0; $f < $found; $f++) {
			...
			$temp->cache_hits_per_sec += (int) $result[5][$f];
			$temp->total_cache_hits += (int) $result[6][$f];
		}
